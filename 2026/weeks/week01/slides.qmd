...
```{r echo=TRUE, eval=TRUE}
modelo_logistico <- glm(pobrezad ~ as_factor(area), data = casen, family = "binomial")
...
La regresión logística revela una **asociación significativa y positiva** entre el área de residencia rural y la pobreza. Específicamente, las personas que residen en áreas rurales tienen **odds de ser pobres `r round(exp(coef(modelo_logistico))[2], 2)` veces mayores** que quienes viven en áreas urbanas (p \< 0,001).
