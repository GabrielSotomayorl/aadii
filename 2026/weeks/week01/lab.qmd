---
title: "Práctico 1: Uso de bases de datos en R, estadística descriptiva y visualización"
subtitle: "Trabajando con CASEN 2024"
date: "2026-03-09"
lang: es
format:
  html:
...
Para esto haremos uso de la encuesta [CASEN (2024)](https://observatorio.ministeriodesarrollosocial.gob.cl/encuesta-casen-2024), la mayor encuesta de hogares realizada en Chile, a cargo del Ministerio de Desarrollo Social, de carácter transversal y multipropósito, es el principal instrumento de medición socioeconómica para el diseño y evaluación de la política social. Permite conocer periódicamente la situación socioeconómica de los hogares y de la población que reside en viviendas particulares, a través de preguntas referidas a composición familiar, educación, salud, vivienda, trabajo e ingresos, entre otros aspectos. 


# 1. Paquetes y funciones en R

Cuando descargamos e instalamos R, este contiene una cantidad limitada de funciones disponibles (lo que llamamos "R Base"). Afortunadamente, podemos ampliar enormemente sus capacidades mediante la descarga de distintos **paquetes** (librerías) creados por la comunidad de usuarios. Estos paquetes nos proporcionan nuevas herramientas para la gestión de bases de datos, análisis estadísticos avanzados y visualización.

En la siguiente práctica trabajaremos con la base de datos de la encuesta **CASEN 2024**. A diferencia de otros formatos genéricos (como Excel o CSV), en este curso trabajaremos con la base en formato **`.RData`**.

::: {.callout-note title="¿Qué es un archivo .RData?"}
Un archivo `.RData` es un formato de guardado **nativo de R**. Su gran ventaja es que permite guardar uno o múltiples objetos (como bases de datos gigantes) comprimidos exactamente como estaban en la memoria de R. Esto hace que carguen muchísimo más rápido y mantengan toda su estructura interna intacta.
:::

## El uso del paquete `haven` y las etiquetas

Aunque cargaremos un archivo nativo de R, **aún necesitamos usar el paquete `haven`**. 

¿Por qué? Porque encuestas oficiales como CASEN suelen ser procesadas originalmente en programas como SPSS o Stata. Al transformar la base a R, las variables conservan una metadata especial llamada **etiquetas** (por ejemplo, guardar la instrucción visual de que un `1` significa *"Hombre"* y un `2` significa *"Mujer"*). El paquete `haven` dota a estas variables de una clase especial llamada `haven_labelled`. Si no cargamos el paquete, R no sabrá cómo interpretar correctamente esta metadata para nuestros análisis.

```{r}
#| label: cargar-haven

# install.packages("haven") 
# Este comando nos permite instalar el paquete desde internet (solo se hace 1 vez).

library(haven) 
# Este comando "enciende" el paquete para usarlo en nuestra sesión actual.
```
...
mean(casen$ytotcorh, na.rm = TRUE) # debemos indicar que ignore los casos perdidos
# mediana
median(casen$ytotcorh, na.rm = TRUE)
# Varianza y desviación estándar
var(casen$ytotcorh, na.rm = TRUE)
sd(casen$ytotcorh, na.rm = TRUE)
# Cuantiles
quantile(casen$ytotcorh, probs = c(0.25, 0.50, 0.75, 0.9), na.rm = TRUE) # percentiles
#minimo
...
#proportions nos da una tabla de proporciones a partir de una tabla
proportions(table(as_factor(casen$sexo)))
...
proportions(table(as_factor(casen$sexo),as_factor(casen$pobreza)),1)
```
...
Por último, necesitamos decirle a ggplot como representar nuestros datos en el eje señalado mediante un geom, es decir, el objeto geométrico que se utilizará para representar los datos. Para esto hay múltiple funciones que empiezan con "geom_", como geom_bar, geom_point o geom_line. En este caso queremos construir un histograma así que usaremos geom_histogram. 
...
Ya tenemos una visualización básica. Ahora introduciremos algunos elementos adicionales para mejorar la apariencia del gráfico:

- **`labs()`**: añade título, subtítulo y nota al pie (`caption`) para contextualizar y citar la fuente de los datos.
- **`theme_minimal()`**: aplica un tema visual limpio y moderno.
- **`theme()`**: permite personalizar en detalle el estilo del título, subtítulo, tamaño de texto de ejes, etc.

```{r,message=FALSE,warning=F}
casen %>%
  ggplot() + 
  aes(x = edad) +
  geom_histogram(fill = "#1a365d", color = "white", bins = 30) +  # Ajusta la cantidad de barras con bins
...
Ahora veamos el ejemplo de un gráfico bivariado. para esto debemos fijar una variable en cada eje para las aesthetics. En este caso veremos la relación entre años de escolaridad y ingresos del trabajo. Para esto usaremos un scatterplot, construido a partir de geom_point().
...
casen %>%
  ...
  geom_smooth(method = "lm", color = "#e67e22", se = FALSE) +  # Añadir una línea de tendencia sin la banda de confianza para claridad
...
ggplot(aes(x = as_factor(sexo))) +
  geom_bar(width = 0.4, fill = "#1a365d") +
...
casen %>% 
ggplot(aes(x = as_factor(sexo))) +
  geom_bar(width = 0.4, fill = "#1a365d", aes(y = after_stat(count/sum(count)))) +
  scale_x_discrete("Sexo") +
...
Luego a partir de esta base de datos construimos el gráfico, ocupando dos geom, geom_point() y geom_line(), para dibujar una línea y puntos en cada edad, para visualizar la tendencia de la pobreza con la edad.
...
casen_pob %>%
  ...
  geom_line(color = "#1a365d") +
  ylim(0, 35) +
  geom_point(shape = 21, fill = "#e67e22", color = "white", size = 1.5) +
  ylab("% Pobreza") +
  xlab("Edad") +
