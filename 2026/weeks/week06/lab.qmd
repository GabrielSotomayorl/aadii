...
pacman::p_load(
              ...
              psych) # Para KMO, Bartlett, Mardia y funciones de AFE
# Cargamos los datos desde la URL
datos <- read.csv("../../data/EDH_2013_csv.csv") %>% 
...
Sx_cov <- cov(datosLW)
n_vars <- ncol(datosLW) # Guardamos número de variables ANTES de agregar columnas auxiliares

# Calcular Distancia de Mahalanobis D^2
D2 <- mahalanobis(datosLW, center = mean_vars, cov = Sx_cov)

# Calcular el p-valor: D^2 sigue distribución chi-cuadrado con df = número de variables (9)
datosLW$p_mahalanobis <- (1 - pchisq(D2, df = n_vars))
...
```
**Interpretación:**
Se identificaron 63 casos como atípicos multivariantes usando un umbral de p < 0.001 para la distancia de Mahalanobis. Estos casos fueron eliminados, resultando en una base de datos final de `r nrow(datosLW)` observaciones para los análisis subsecuentes.
...
# Test de Mardia usando el paquete psych (ya cargado)
# Usaremos datosLW (que ya no tiene NAs ni atípicos multivariantes)
resultados_mardia <- psych::mardia(datosLW, plot = TRUE)
print(resultados_mardia)

# Normalidad univariante con Shapiro-Wilk por variable
cat("\n--- Test de Shapiro-Wilk por variable ---\n")
normalidad_uni <- sapply(datosLW, function(x) {
  t <- shapiro.test(x)
  c(W = round(t$statistic, 4), p.value = round(t$p.value, 6))
})
print(t(normalidad_uni))
```
**Interpretación:**
*   **Normalidad Multivariante (Test de Mardia):**
    *   `b1p` (Asimetría de Mardia): p-valor ≈ 0. Resultado: **NO** normal.
    *   `b2p` (Curtosis de Mardia): p-valor ≈ 0. Resultado: **NO** normal.
    *   **Conclusión:** Los tests de Mardia indican claramente que los datos **no cumplen con el supuesto de normalidad multivariante**. El gráfico Q-Q chi-cuadrado también muestra desviación de la línea diagonal.
*   **Normalidad Univariante (Test de Shapiro-Wilk):**
    *   Para **todas y cada una de las 9 variables**, el p-valor < 0.001, indicando que **ninguna de las variables individuales sigue una distribución normal**.
