...
Los datos **no cumplen con el supuesto de normalidad multivariante**. Además, los indicadores Likert de las variables latentes son ordinales. Por ello, usaremos el estimador **`DWLS`** (Diagonally Weighted Least Squares) en `lavaan`, que trabaja con correlaciones policóricas subyacentes para este tipo de indicadores.
...
* **Test $\chi^2$:** El estadístico robusto es **2407.265** con **61** grados de libertad (p-valor = 0.000). Al trabajar con una muestra de gran tamaño ($N > 10.000$), la prueba de $\chi^2$ es sensible al tamaño muestral, por lo que conviene complementar esta evaluación con otros índices descriptivos de ajuste.
* **CFI = 0.986; TLI = 0.994:** Ambos valores superan el heurístico convencional de 0.95, lo que sugiere buen ajuste comparativo en relación con el modelo nulo de independencia.
* **RMSEA = 0.060 (IC 90%: 0.058 - 0.062):** Este valor se sitúa dentro del rango considerado como de **ajuste aceptable** (entre 0.05 y 0.08).
* **SRMR = 0.024:** Este valor se ubica bajo la guía sugerida de 0.08.

**Conclusión de Ajuste Global:** En general, los índices CFI, TLI, RMSEA y SRMR sugieren que la estructura de covarianzas implicada por el modelo es compatible con las relaciones observadas en la muestra. Esta lectura debe complementarse con la evaluación del ajuste local.

**B. Interpretación del Modelo de Medida (Cargas Factoriales `Std.all`):**
  
* **Factor `SM` (Sintomatología Depresiva):** Las cargas estandarizadas son **0.771, 0.890, 0.806, y 0.746**. Los indicadores Likert presentan cargas altas y estadísticamente significativas (p<0.001).
* **Factor `SEG` (Percepción de Seguridad):** Las cargas estandarizadas son **0.893, 0.918, 0.759, y 0.555**. Los ítems sobre seguridad caminando de día (`seg_2`) y plazas/parques (`seg_1`) tienen mayor peso en la medición del constructo. La seguridad caminando de noche (`seg_3`) también aporta al factor, mientras que la seguridad dentro de la vivienda (`seg_4`) presenta la carga más baja (0.555).

**C. Interpretación del Modelo Estructural (Regressions `Std.all`):**

* **`qaut` (Quintil de Ingresos):** Beta = **-0.065**. A mayor quintil de ingresos, se observa una disminución de la sintomatología depresiva, controlando por las demás variables.
...
* **`sexo` (Mujer=1):** Beta = **0.175**. Las mujeres muestran una mayor sintomatología depresiva en comparación con los hombres, manteniendo el resto de las variables constantes.
...
* **`maltrato` (Tratamiento Injusto = 1):** Beta = **0.254**. Reportar haber sufrido tratamiento injusto o discriminación durante el último año es el predictor positivo de mayor magnitud en este modelo.
...
Al analizar la matriz de residuos de correlación policórica, observamos que las discrepancias específicas son pequeñas. El residuo absoluto más alto es **-0.083** (entre `seg_3` "seguridad caminando de noche" y `seg_4` "seguridad dentro de la vivienda"). Como ningún par de indicadores excede el límite heurístico de **$|0.10|$**, no se observan desajustes locales grandes según este criterio.
...
1. **Especificado y estimado** un Modelo de Ecuaciones Estructurales (SEM) en R, aplicando `DWLS` por la naturaleza ordinal de los indicadores.
2. **Evaluado el ajuste global**, encontrando índices descriptivos compatibles con el modelo propuesto (CFI = 0.986, TLI = 0.994, RMSEA = 0.060, SRMR = 0.024).
3. **Revisado localmente el ajuste**, observando que ningún residuo de correlación absoluta supera el heurístico de $|0.10|$.
4. **Analizado el modelo de medida**, observando que las escalas de sintomatología depresiva (`SM`) y percepción de seguridad (`SEG`) son representadas por sus indicadores.
...
6. Cuantificado que este sistema estructural explica un **18.0% de la varianza** en el constructo de sintomatología depresiva latente.

### Nota sobre Inferencia Causal y Modelos Equivalentes (Kline, 2023)

Al trabajar con datos transversales (cross-sectional) de la Encuesta de Bienestar Social (EBS) 2023, la estimación de un coeficiente de trayectoria negativo de `SEG` sobre `SM` (Percepción de Seguridad ➡️ Sintomatología Depresiva) debe leerse como una asociación estadística de coincidencia temporal.
