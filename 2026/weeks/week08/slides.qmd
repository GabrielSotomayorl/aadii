...
-   Los umbrales dependen del N, el número de indicadores y la complejidad del modelo
-   **Un modelo puede tener CFI > 0.95 y aun así ocultar errores graves de especificación** (Kline, 2023)
-   El ajuste global no dice *dónde* ni *cómo* falla el modelo
-   Los índices globales son evidencia necesaria pero no suficiente — siempre deben complementarse con el ajuste **local**

## Ajuste Local: Residuos de Correlación {.smaller}

-   El ajuste global promedia el ajuste de todo el modelo — puede esconder problemas específicos
-   La **Matriz de Residuos de Correlación** muestra las diferencias entre:
    -   Correlaciones **observadas** en los datos
    -   Correlaciones **reproducidas** por el modelo
-   **Residuos absolutos \> 0.10** → problema grave de ajuste local entre ese par de ítems
-   Kline (2023): es la herramienta más informativa para detectar errores de especificación

## Ajuste Local: Código en `lavaan` {.smaller}
...
-   Las **cargas factoriales estandarizadas** miden la asociación entre el indicador y la variable latente
    -   Buen modelo: cargas ≥ 0,7 (mínimo aceptable: 0,5)
-   Las **comunalidades** son el R² de cada indicador: proporción de su varianza explicada por el factor
-   Cargas más altas → mediciones más “puras” del constructo
-   El indicador con carga más baja es candidato a eliminar si es necesario reducir la escala
...
## Reespecificar modelo: HARKing {.smaller}

:::{.callout-warning}
## Capitalización del azar
Agregar correlaciones de error *solo* porque un IM es alto transforma el AFC en análisis exploratorio.
:::

-   **HARKing** (*Hypothesizing After the Results are Known*): postular hipótesis *después* de ver los datos
-   Al modificar mecánicamente según los IM:
    -   **Capitalizas el azar**: ajustas a *esta* muestra, no a la teoría
    -   Los índices mejoran artificialmente — más parámetros, no mejor modelo
    -   El AFC pierde su carácter **confirmatorio**
-   Kline (2023): *"Parameter changes suggested by MIs that lack theoretical or logical justifications should not be made"*

## Reespecificar modelo: Reglas de oro {.smaller}

1.  Toda modificación requiere **justificación sustantiva y teórica previa** — no solo un IM alto
2.  Correlacionar errores exige explicar por qué dos ítems comparten varianza *más allá* del factor (ej. redacción similar, mismo método, mismo momento)
3.  Si el modelo se modifica guiado por los datos: **validar en muestra independiente** antes de reportarlo como resultado confirmatorio
...
## Ítems Ordinales en `lavaan` {.smaller}

-   `lavaan` permite declarar variables ordinales directamente en `cfa()` con el argumento `ordered`
-   Usar siempre el estimador robusto **WLSMV** (`estimator = "WLSMV"`) para datos categóricos
