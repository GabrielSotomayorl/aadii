...
**Interpretación (Tabla 1):** El coeficiente para "No heterosexual" es $`r format(b_os_m1, big.mark=",")`$ pesos (p = `r round(p_os_m1, 2)`): prácticamente nulo y no significativo. Sin controlar por ninguna otra variable, la orientación sexual no predice el ingreso laboral. Esto no significa que no haya brecha —significa que la educación más alta de las personas no heterosexuales **compensan** su desventaja estructural, haciéndola invisible a nivel descriptivo.
