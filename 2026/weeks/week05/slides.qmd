...
title: "<font style='font-size:1em;'>🗓️ Semana 05<br/>Introducción a los Modelos Multivariados</font>"
...
## Funciones de los Modelos en Cs. Sociales {.smaller}
...
## Un Mapa para Navegar los Modelos {.smaller}
...
## Eje 1: Exploratorio vs. Confirmatorio  {.smaller}
...
## Eje 2: Medición vs. Relacional {.smaller}
...
## Variabilidad y Dispersión {.smaller}
...
ggplot(datos[order(datos$brecha), ], aes(x = seq_along(brecha), y = brecha)) +
  ...
  geom_hline(yintercept = mean_brecha, color = "red", linetype = "dashed", linewidth = 1) +  # Línea de la media
  geom_segment(aes(xend = seq_along(brecha), yend = mean_brecha), color = "#ffa500", linewidth = 0.5) +  # Líneas desde cada punto hasta la media
  labs(
    x = "Índice",
...
## Co-Variación: Covarianza y Correlación {.smaller}
...
## Correlaciones para Datos No Continuos {.smaller}
...
*(En R, el paquete `psych` permite calcularlas con las funciones `polychoric()` y `tetrachoric()`).*

# Parte III: De la Correlación a la Regresión Lineal

## Más Allá de la Asociación: Regresión Lineal Simple (RLS) {.smaller}
...
ggplot(datos, aes(x = promedio_anios_escolaridad25_2017, y = brecha)) +
  ...
  geom_vline(xintercept = mean(datos$promedio_anios_escolaridad25_2017, na.rm = TRUE), color = "red", linetype = "dotted", linewidth = 1) +  # Línea roja punteada en la media de 'promedio_anios_escolaridad25_2017'
  geom_hline(yintercept = mean(datos$brecha, na.rm = TRUE), color = "red", linetype = "dotted", linewidth = 1) +  # Línea roja punteada en la media de 'brecha'
  labs(
...
## ¿Cómo Encontrar la "Mejor" Recta? Mínimos Cuadrados (OLS) {.smaller}
...
![](img/resid.jpg){fig-align="center"}

## Fórmulas para OLS (RLS) {.smaller}

Para calcular la pendiente 𝑏 y la ordenada en el origen 𝑎, se utilizan las siguientes fórmulas: $$
b = r \frac{s_y}{s_x}
$$
...
## Análisis de Residuos: Evaluando el Ajuste del Modelo {.smaller}
...
ggplot(datos, aes(x = predicciones, y = residuos)) +
  geom_point(color = "#0073C2", size = 3, alpha = 0.7) +  # Puntos más grandes y ligeramente transparentes
  geom_hline(yintercept = 0, color = "red", linetype = "dotted", linewidth = 1) +  # Línea en y=0 para referencia
  labs(
    x = "Valores Predichos",
    y = "Residuos",
...
## Interpretando el Gráfico de Residuos {.smaller}
...
## Diagramas de residuos {.smaller}

![](img/residuos.jpg)


## Bondad de Ajuste: Coeficiente de Determinación ($R^2$) {.smaller}
...
*   Varía entre 0 y 1. Un $R^2$ de 0.13 indica que el 13% de la varianza total de Y es explicada por el modelo.

## Limitaciones de RLS y la Necesidad de RLM {.smaller}
...
## Regresión Lineal Múltiple (RLM): El Poder del Control {.smaller}
...
## Interpretando Coeficientes Parciales en RLM ($b_j$) {.smaller}
...
**Ejemplo:** $\text{Ingreso} = b_0 + b_1 \times \text{Escolaridad (años)} + b_2 \times \text{Edad (años)}$
...
## Predictores Categóricos en RLM: Variables Dummy {.smaller}
...
## Comparando Efectos: Coeficientes Estandarizados (β) {.smaller}
...
## Ajuste del Modelo RLM: $R^2$ y $R^2$ Ajustado {.smaller}
...
## Inferencia: De la Muestra a la Población  {.smaller}
...
## Test de Hipótesis para Coeficientes ($b_j$) {.smaller}
...
## Interpretación y Decisión {.smaller}
...
*   Los SE estarán **incorrectos**: generalmente subestimados por el efecto de conglomerado (las observaciones dentro de un mismo conglomerado están correlacionadas, reduciendo el tamaño muestral efectivo).
*   Los p-valores resultantes también estarán **incorrectos**.
*   **Esto puede llevar a conclusiones falsas sobre la significancia**
...
En el práctico aplicaremos estos conceptos en R usando `survey::svyglm()`:

1.  Ajuste de modelos RLS y RLM incorporando el diseño muestral complejo.
2.  Interpretación de coeficientes parciales para predictores continuos y categóricos.
3.  Cálculo manual de coeficientes estandarizados (Betas).
4.  Diagnósticos del modelo: linealidad, homocedasticidad, normalidad de residuos y VIF.
