...
*   **Normalidad:**
    ...
    *   **Implicancia para la selección del estimador (Kline, 2023):** Dado que las variables dependientes que estamos modelando (`castigo_media` y `rwa_media`) son **índices promedio continuos** y no cumplen normalidad multivariante, usaremos **MLR (Maximum Likelihood Robust / Máxima Verosimilitud Robusta)**. Este estimador utiliza el algoritmo estándar de Máxima Verosimilitud (ML), pero aplica correcciones robustas (tipo Sandwich y correcciones de escala tipo Yuan-Bentler) a los errores estándar, valores *p* e índices de ajuste global.
