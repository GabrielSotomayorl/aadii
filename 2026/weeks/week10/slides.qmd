...
## Los 6 Pasos Fundamentales del Path Analysis {.smaller}
...
*   **Métodos de Estimación Comunes:**
    ...
    *   **ML Robusto (MLR):** El estándar de oro moderno para variables **continuas** que violan la normalidad. Aplica correcciones robustas (Satorra-Bentler) a los errores estándar y al estadístico $\chi^2$.
    *   **DWLS / WLSMV:** Estimador robusto diseñado **exclusivamente para variables ordinales/categóricas** (nunca usar con índices continuos promediados).
</div>
...
*   **Outcome (variable endógena) dicotómico** (ej. pobreza sí/no, vota/no vota): requiere un tratamiento distinto al de los predictores, ya que condiciona el estimador y la interpretación de todos los coeficientes del modelo.
...
## Outcome dicotómico: por qué ML/MLR no es adecuado {.smaller}

<div style="font-size: 0.9em; line-height: 1.2;">

Cuando la variable endógena es dicotómica (0/1), estimar el modelo con ML o MLR produce resultados incorrectos:
...
*   El modelo se estimará sin errores en R, pero los resultados no son válidos.

La solución es declarar el outcome como variable ordenada (`ordered`) y usar el estimador **WLSMV**, que trata la variable dicotómica como la manifestación observable de una **variable latente continua subyacente** $Y^*$, cortada por un umbral $\tau$:
